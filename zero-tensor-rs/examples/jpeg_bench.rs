...
fn generate_dataset(dir: &Path, num_images: usize, num_classes: usize) {
    println!("[Rust] Generating {} images in {} classes...", num_images, num_classes);
...
fn main() {
    ...
    println!("[Rust] Slot size: {} bytes ({:.2} MB)", 
             slot_size, slot_size as f64 / 1024.0 / 1024.0);
    ...
    let dataset = JpegFolderDataset::new(
        &dataset_dir,
        label_fn,
        TensorDT::F32,
    ).expect("Failed to create dataset");
    let init_time = start_init.elapsed();
    println!("[Rust] Dataset initialized in {:.2}s ({} images)", init_time.as_secs_f32(), dataset.len());
    ...
    println!("[Rust] Streaming {} batches of size {}...", num_images / batch_size, batch_size);
    ...
    println!("[Rust] Streaming completed in {:.2}s", stream_time.as_secs_f32());
    println!("[Rust] Done!");
