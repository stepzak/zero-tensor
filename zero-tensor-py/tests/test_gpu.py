...
class TestToDeviceEventRecording:
    ...
    def test_non_blocking_records_event(self, temp_ipc_env):
        ...
        try:
            with MockedGPU():
                with ZeroTensorConsumer(socket_path, shm_name) as consumer:
                    for batch_tensor in consumer:
                        consumer.to_device(batch_tensor, device='cuda', non_blocking=True)
    ...
    def test_blocking_does_not_record_event(self, temp_ipc_env):
        ...
        try:
            with MockedGPU():
                with ZeroTensorConsumer(socket_path, shm_name) as consumer:
                    for batch_tensor in consumer:
                        consumer.to_device(batch_tensor, device='cuda', non_blocking=False)
        ...
        try:
            with MockedGPU():
                with ZeroTensorConsumer(socket_path, shm_name) as consumer:
                    for batch_tensor in consumer:
                        consumer.to_device(
                            batch_tensor, device='cuda', non_blocking=True, copy=True
                        )
                ...
                with ZeroTensorConsumer(socket_path, shm_name) as consumer:
                    for batch_tensor in consumer:
                        result = consumer.to_device(
                            batch_tensor, device='cuda', non_blocking=True, dtype=torch.float16
                        )

                        assert result.data_ptr() != batch_tensor.data_ptr()
        ...
        try:
            with MockedGPU():
                with ZeroTensorConsumer(socket_path, shm_name) as consumer:
                    ...
                    count = 0
                    for batch_tensor in consumer:
                        gpu_batch = consumer.to_device(
                            batch_tensor, device='cuda', non_blocking=True
                        )

                        if prev_batch is not None:
                            _ = prev_batch.sum().item()
    ...
    def test_to_device_without_gpu_returns_tensor(self, temp_ipc_env):
        ...
        try:
            with patch('torch.cuda.is_available', return_value=False):
                with ZeroTensorConsumer(socket_path, shm_name) as consumer:
                    for batch_tensor in consumer:
                        result = consumer.to_device(batch_tensor, device='cpu')
                        assert result is not None
                        assert torch.allclose(result, batch_tensor)
